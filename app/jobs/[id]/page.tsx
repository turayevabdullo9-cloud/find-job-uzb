...
const JobDetailsPage = () => {
    ...
    useEffect(() => {
        const fetchJob = async () => {
            ...
            try {
                const { data } = await axios.get(`http://localhost:3001/jobs/${params.id}`);
                setJob(data);
            } catch {
                const fallback = db.jobs.find((item) => String(item.id) === String(params.id));
    ...
    if (isLoading) {
        return (
            <div className="min-h-screen bg-slate-50 py-20">
                <div className="container mx-auto px-6 text-center text-slate-700">Загрузка вакансии...</div>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-slate-50 py-10">
            <div className="container mx-auto px-6">
                <Link href="/jobs" className="inline-flex items-center rounded-full border border-slate-300 bg-white px-4 py-2 text-sm text-slate-700 transition hover:bg-slate-100">
                    ← Back to Jobs
                </Link>
                    ...
                    <div className="space-y-6 rounded-3xl bg-white p-8 shadow-lg">
                        <div className="flex flex-col gap-4 rounded-3xl border border-slate-200 p-6">
                            <div className="flex items-start justify-between gap-4">
                                ...
                                <div className="rounded-3xl bg-slate-950 px-5 py-4 text-xl font-bold text-white">T</div>
                            </div>
                            <div className="flex flex-wrap gap-3">
                                <span className="rounded-full bg-slate-100 px-3 py-2 text-sm text-slate-700">{job.category}</span>
                                <span className="rounded-full bg-slate-100 px-3 py-2 text-sm text-slate-700">{job.location}</span>
                            </div>
                            <div className="flex flex-wrap items-center justify-between gap-4 border-t border-slate-200 pt-4">
                                <div className="text-sm text-slate-500">{job.type}</div>
                                <div className="text-lg font-semibold text-amber-700">{job.salary}</div>
                            </div>
                            <button
                        ...
                        <div className="grid gap-6 md:grid-cols-2">
                            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-6">
                                <h2 className="mb-4 text-lg font-semibold text-slate-950">Job Description</h2>
                                <p className="text-sm leading-7 text-slate-600">{job.description}</p>
                            </div>
                            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-6">
                                <h2 className="mb-4 text-lg font-semibold text-slate-950">Job Details</h2>
                        ...
                        <div className="rounded-3xl border border-slate-200 bg-white p-6">
                            <h2 className="mb-4 text-lg font-semibold text-slate-950">Requirements</h2>
                    ...
                    <div className="space-y-6">
                        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                            <div className="mb-4 flex items-center justify-between gap-3">
                                ...
                                <div className="rounded-full bg-slate-100 px-4 py-2 text-sm text-slate-700">{job.type}</div>
                            </div>
                ...
                <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/70 px-4 py-8">
                    <div className="w-full max-w-xl rounded-3xl bg-white p-8 shadow-2xl">
                        <div className="mb-6 flex items-center justify-between gap-4">
                            <div>
                                <h2 className="text-2xl font-bold text-slate-950">Apply for this role</h2>
                                <p className="text-sm text-slate-500">Submit your application and the admin will see it in Applications.</p>
                            </div>
                            <button
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            ...
                            <button type="submit" className="w-full rounded-2xl bg-blue-900 px-6 py-3 text-sm font-semibold text-white transition hover:bg-blue-800">
                                Submit Application
                            </button>
                        </form>
                    </div>
